...
public class Integrasi {
	...
	public void bentukGraph() throws Exception {
		...
		for (OSMNode tempat : allTempat) {
			Vertex lokasi = new Vertex(tempat.getId(),
					Integer.toString(banyakTempat));
			vertexs.add(lokasi);
			banyakTempat++;
		}
		...
		for (Edge e : edges) {
			System.out.println("ID : " + e.getId() + " Jarak : "
					+ e.getWeight());
		}
		...
		// LinkedList<Vertex> path = dijkstra.getPath(vertexs.get(4)); //cari id
		// di value
		// LinkedList<Vertex> path = dijkstra.getPath(vertexs.get(4)); //cari id
		// di value
		LinkedList<Vertex> path = dijkstra.getPath(vertexs.get(23)); // cari id
																		// di
																		// value
		if (path == null) {
	...
	private int kalkulasiPanjang(OSMNode n1, OSMNode n2) {
		// TODO Auto-generated method stub
		double length = LatLongUtil.distance(Double.parseDouble(n1.lat),
	...
	private void addJarakAntarNode(String laneId, int sourceLocNo,
		...
		Edge lane = new Edge(laneId, vertexs.get(sourceLocNo),
				vertexs.get(destLocNo), duration);
		edges.add(lane);
	}
